...
| `ErrorModel` (`FTELSRCore.Shared/Models/Https/ErrorModel.cs`) | Chua `Code` (int), `Message` (string), `Succeeded` (bool), kem method `ErrorDeConstruct(out string message, out int statusCode)` gan `message = Message`, `statusCode = Code` |
...
| 25 | Da doi chieu doc lap voi module Models - Https (`ErrorModel.cs:11-15`): muc 1.2 truoc day mo ta `ErrorModel` chi co 3 property, thieu method `ErrorDeConstruct(out string message, out int statusCode)` — da bo sung dong mo ta o muc 1.2 | `FTELSRCore.Shared/Models/Https/ErrorModel.cs:11-15` | Thap. Chi anh huong do day du cua tai lieu, khong anh huong hanh vi runtime |
